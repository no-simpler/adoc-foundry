...
// Open file, inject mathjax, render math
...
var removeMathJaxScripts = function() {
    ...
    // Does 'str' contain 'infix'?
    var strContains = function(str, infix) {
    ...
    while (MathJaxScriptFound) {
        ...
        for (var i = scripts.length - 1; i >= 0; i--) {
            if (scripts[i].hasAttribute("id")) {
                ...
                // Remove script if 'src' ends on 'with-(svg|html).js', 
                // or 'jax.js' or contains 'MathJax.js'
                if ((strEndsWith(src, "with-svg.js")) 
                    || (strEndsWith(src, "with-html.js")) 
                    || (strEndsWith(src, "jax.js"))
                    || (strContains(src, "MathJax.js"))) { 
                    MathJaxScriptFound = true;
                    scripts[i].parentNode.removeChild(scripts[i]);
                };
            } else if (scripts[i].hasAttribute("type")) {
                var type = scripts[i].getAttribute("type");
                // Remove script if 'type' ends on 'x-mathjax-config'
                if (strContains(type, "x-mathjax-config")) {
                    MathJaxScriptFound = true;
                    scripts[i].parentNode.removeChild(scripts[i]);
                };
            };
